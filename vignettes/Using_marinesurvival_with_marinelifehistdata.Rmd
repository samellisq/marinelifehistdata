...
vignette: >
  %\VignetteIndexEntry{Using_marinesurvival_with_marinelifehistdata}
...
The code and data can then be combined in either rstan or cmdstanr- here we use rstan:
...
Ta da, model fit as an ordinary bayesian model. The summary data can be accessed but tends to be rather long becasue of all the estimates of unknown ages. The marinesurvival pacakge has a function called get_activeparameters to identify the non-age parameters present in the model which can then be used to somplfy the output. 
